...
\def\MM{\ensuremath{\varmathbb{M}}}
\def\CC{\ensuremath{\varmathbb{C}}}
...
$\F_q$-endomorphism $\pi: \K \rightarrow \K$ defined by $x \mapsto x^q$ is called the Frobenius 
...
integers $j_1, r_1$ such that $j_1 / r_1 = j / r$. Repeating this process, we obtain another pair 
$j_2, r_2$. It can be shown that $r = \lcm(r_1, r_2)$ with probability $\approx 6 / \pi^2$. 
Therefore, we only need to run the above procedure a constant number of times to obtain $r$ with 
...
\begin{proposition}
	...
	Frobenius of $\F_q[x] / f$, and an integer $d = p_1^{e_1} p_2^{e_2} \cdots p_\ell^{e_\ell}$ in 
