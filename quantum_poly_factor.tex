...
\begin{proof}
	...
	n})$. For this we use a subproduct tree.
	
	Let $P = \{ q_1, q_2, \dots, q_s \}$ be the set of all primes $\le n$. The subproduct tree for 
	the set $P$ is a binary tree described as follows. The roots of the tree is the product $q_1 
	q_2 \dots q_s$. Now divide the set $P$ into two halves for the left and right subtrees with 
	roots the products $q_1 q_2 \dots q_{\lceil s / 2 \rceil}$ and $q_{\lceil s / 2 \rceil + 1} 
	\dots q_s$ respectively. Repeating this this process results in a tree with leaves the primes 
	in $P$. The tree can be built recursively: the value of a parent node is the product of the 
	roots of the left and right subtrees. Assuming two $m$-bit integers are multiplied in $O(m^{1 + 
...
\begin{proof}
	...
	$O(2^{\log^2n})$ then it takes $O(m^{1 + o(1)} (\log^cn) \log^{1 + o(1)}q)$ bit operations. 
	Otherwise it takes $O(m^{4 / 3 + o(1)} \log^{2 + o(1)}q)$ bit operations. 
	
	Since $d$ is smooth, Step \ref{step:factor-d} can be efficiently done. More precisely, we have 
	$p_i^{e_i} \le n$. 
